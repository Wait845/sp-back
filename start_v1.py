...
def enter_handle():
    ...
    # start_time = datetime.fromtimestamp(start_time)
    start_time = datetime.strptime(datetime.now().strftime("%Y-%m-%d"), "%Y-%m-%d")
    end_time = datetime.fromtimestamp(end_time)
...
def set_overflow_num():
    ...
    # print(sql_record_num)
    # print(dao.execute(sql_record_num))
    return ""
    ...
    if request.method == "POST":
        ...
        for data in data_list:
            ...
            if None in [interface, addr, rssi, frequency, channel, timestamp, probe_type, distance]:
                continue
            # filter ignore address
            if addr in ig_addr:
                continue
            # filter disable wlan sensor
            if interface in utils.sensor_filter_list:
                continue

            # cover timestamp to str
            timestamp = datetime.fromtimestamp(timestamp)
            # start_time is earliest time of these msg list
            if start_time == None:
        ...
        if sql_record_result != None:
            utils.calculate_stream(start_time, 120)
